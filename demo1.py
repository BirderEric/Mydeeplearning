    ...
    try:
        for step in np.arange(MAX_STEP):
            ...
            if step % 128 == 0 or (step + 1) == MAX_STEP:
                print("Step: %d, loss: %.8f, accuracy: %.4f%%" % (step, train_loss, train_accuracy))
            ...
            if step % 128 == 0 or (step + 1) == MAX_STEP:
                ...
                print("** Step: %d, loss: %.8f, test_accuracy: %.4f%%" % (step, val_loss, val_accuracy))
                summary_str = sess.run(summary_op)
...
def train():
    ...
    logits = VGG.MyResNet(train_image_batch, N_CLASSES, IS_PRETRAIN)
    loss = tools.loss(logits, train_label_batch)
    accuracy = tools.accuracy(logits, train_label_batch)
    my_global_step = tf.Variable(0, trainable=False, name='global_step')
    train_op = tools.optimize(loss, learning_rate, my_global_step)
